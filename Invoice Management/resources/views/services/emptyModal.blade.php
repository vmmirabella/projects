		...
		$.ajax({
		  ...
		  success: function(data){
			...
			if(editRow == false){
				...
				'<button type="button" id="edit-form-btn" class="btn btn-default" href="services/'+ data['id'] + '/edit" data-toggle="modal" data-target="#edit-form">Edit</button> <button type="button" id="view-form-btn" class="btn btn-info" href="services/'+ data['id'] + '" data-toggle="modal" data-target="#view-form">View</button> <button type="button" id="delete-form-btn" class="btn btn-danger" data-href="services/'+ data['id'] + '/destroy" data-toggle="modal" data-target="#confirm-delete">Delete</button>' +
				'</td></tr>'
				);
			}
			...
			rowToEdit.find('td:nth-child(1)').html(data['name']);
			rowToEdit.find('td:nth-child(2)').html(data['shortDescription']);
			data['flatRate'] = parseFloat(data['flatRate']).toFixed(2);
			rowToEdit.find('td:nth-child(3)').html(data['flatRate']);
			data['hourlyRate'] = parseFloat(data['hourlyRate']).toFixed(2);
			rowToEdit.find('td:nth-child(4)').html(data['hourlyRate']);
